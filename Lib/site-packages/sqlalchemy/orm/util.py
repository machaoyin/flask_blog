...
class AliasedClass(object):
    ...
    mapped model can be used in multiple contexts.   A simple example::
...
class AliasedInsp(InspectionAttr):
    ...
    * ``model`` - the :class:`.AliasedClass` represented.
...
def aliased(element, alias=None, name=None, flat=False, adapt_on_names=False):
    ...
    :param adapt_on_names: if True, more liberal "matching" will be used when
     mapping the mapped columns of the ORM model to those of the
     given selectable - a name-based match will be performed if the
     given selectable doesn't otherwise have a column that corresponds
     to one on the model.  The use case for this is when associating
     an model with some derived selectable such as one that uses
     aggregate functions::
...
def with_parent(instance, prop, from_entity=None):
    """Create filtering criterion that relates this query's primary model
...
def _entity_corresponds_to(given, entity):
    """determine if 'given' corresponds to 'model', in terms
    of an model passed to Query that would match the same model
