...
class SubqueryLoader(AbstractRelationshipLoader):
    ...
    def setup_query(
        ...
        **kwargs
        ...
        # model to the thing we're subquery loading.
    ...
    def _generate_from_original_query(
        ...
        # other columns from the query that might suggest the right model
    ...
    def _create_eager_join(
        ...
        if (
            ...
            if len(indexes) > 1:
                ...
                raise sa_exc.InvalidRequestError(
                    "Can't identify which query model in which to joined "
        ...
        if adapter:
            if getattr(adapter, "aliased_class", None):
                # joining from an adapted model.  The adapted model
                # might be a "with_polymorphic", so resolve that to our
                # specific mapper's model before looking for our attribute
                # name on it.
                ...
                # look for our attribute on the adapted model, else fall back
    ...
    def _create_scalar_loader(self, context, key, _instance, populators):
        ...
        def load_scalar_from_joined_existing_row(state, dict_, row):
            ...
            if key in dict_:
                ...
                # same model (e.g. via aliasing), one has an attribute
...
class SelectInLoader(AbstractRelationshipLoader, util.MemoizedSlots):
    ...
    def _load_for_path(
        ...
        if not query_info.load_with_join:
            ...
            # "in" expression are in terms of the related model.  So
            # if the related model is polymorphic or otherwise aliased,
            # we need to adapt our "pk_cols" and "in_expr" to that
            # model.   in non-"omit join" mode, these are against the
            # parent model and do not need adaption.
        ...
        if not query_info.load_with_join:
            ...
            # model, we add it explicitly.  If we made the Bundle against
            ...
            # mapped column of the parent model, but the #4347 issue does not
            # occur in this case.
