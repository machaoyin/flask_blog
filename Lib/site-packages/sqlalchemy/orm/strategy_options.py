...
class Load(Generative, MapperOption):
    ...
    where it
    will only take effect for the ``MyClass`` model::
        ...
        for (key, loader_path), obj in self.context.items():
            ...
            for local_elem, obj_elem in zip(self.path.path, loader_path):
                ...
                if (
                    ...
                    # into an alias or other unsafe model
                    not chopped
    ...
    def _generate_path(
        ...
        if raiseerr and not path.has_entity:
            if isinstance(path, TokenRegistry):
                raise sa_exc.ArgumentError(
                    "Wildcard token cannot be followed by another model"
                )
            else:
                raise sa_exc.ArgumentError(
                    "Mapped attribute '%s' does not "
                    "refer to a mapped model" % (path.prop,)
                )
        ...
        if isinstance(attr, util.string_types):
            ...
            if attr.endswith(_WILDCARD_TOKEN) or default_token:
                ...
                # working for a with_polymorphic model because the
...
class _UnboundLoad(Load):
    """Represent a loader option that isn't tied to a root model.

    The loader option will produce an model-linked :class:`_orm.Load`
    ...
    def _from_keys(cls, meth, keys, chained, kw):
        ...
        def _split_key(key):
            if isinstance(key, util.string_types):
                ...
                # coerce fooload(".*") into "wildcard on default model"
    ...
    def _bind_loader(self, entities, current_path, context, raiseerr):
        ...
        refer to a lead model as it may use string tokens.   The Load()
        ...
        # what model we are referring towards.
        token = start_path[0]
        ...
        # transfer our model-less state into a Load() object
        # with a real model path.  Start with the lead model
            ...
            if len(list(entities)) != 1:
                if raiseerr:
                    raise sa_exc.ArgumentError(
                        ...
                        "loader options for each model individually, such "
                        "as %s."
                        % (
...
def contains_eager(loadopt, attr, alias=None):
    ...
    The above query would join from the ``Order`` model to its related
    ``User`` model, and the returned ``Order`` objects would have the
...
def load_only(loadopt, *attrs):
    """Indicate that for a particular model, only the given list
...
def undefer(loadopt, key):
    ...
    e.g. specified within the SELECT statement of the model as a whole.
...
def undefer_group(loadopt, name):
    ...
    To undefer a group of attributes on a related model, the path can be
