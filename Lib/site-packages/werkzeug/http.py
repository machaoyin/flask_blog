...
def remove_entity_headers(headers, allowed=("expires", "content-location")):
    """Remove all model headers from a list or :class:`Headers` object.  This
    operation works in-place.  `Expires` and `Content-Location` headers are
    by default not removed.  The reason for this is :rfc:`2616` section
    10.3.5 which specifies some model headers that should be sent.
...
def is_entity_header(header):
    """Check if a header is an model header.

    .. versionadded:: 0.5

    :param header: the header to test.
    :return: `True` if it's an model header, `False` otherwise.
    """
